...
public class RoaringBitmapMinimum extends AbstractGenericUDFRoaringBitmapBase{

    @Override
    public ObjectInspector initialize(ObjectInspector[] objectInspectors) throws UDFArgumentException {
        initFunctionParameters("rb_minimum",1,1,ObjectInspector.Category.PRIMITIVE,"bigint");
    ...
    public Object evaluate(DeferredObject[] deferredObjects) throws HiveException {
        ...
        if (bitmap.last()>=0&&bitmap.first()>=0) {
            return new LongWritable(bitmap.first());
        }
        if (bitmap.first()<0&&bitmap.last()<0) {
            return new LongWritable(bitmap.first());
        }
        return new LongWritable(bitmap.stream().min().getAsLong());
    }
}
